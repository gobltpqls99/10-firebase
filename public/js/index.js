...
function onDelete(el) {
	...
	db.ref('root/note-sample/'+key).remove();
}
...
function onSubmit(f) {
	...
	if(f.title.value !== '') db.ref('root/note-sample').push(data);
	f.title.value = '';
	return false;
}

function dbInit() {
	db.ref('root/note-sample').on('child_added', onAdd);
	db.ref('root/note-sample').on('child_removed', onRev);
}
